...
void SimulationEngine::repelParticles(std::vector<Particle>& particles_1, const std::vector<Particle>& particles_2, float force, float radius) {
...
void SimulationEngine::step() {
    for (unsigned i = 0; i < 4; i++) {
        for (unsigned j = 0; j < 4; j++) {
            ...
            repelParticles(*c1, *c2, forces[i][j].force, forces[i][j].radius);
        }
    }
}
...
void SimulationEngine::attractToHelper(std::vector<Particle>& particles, sf::Vector2f pos, float radius, float force) {
...
void SimulationEngine::attractTo(sf::Vector2f pos, float force, float radius) {
    attractToHelper(particles_r, pos, radius, force);
    attractToHelper(particles_g, pos, radius, force);
    attractToHelper(particles_b, pos, radius, force);
    attractToHelper(particles_y, pos, radius, force);
}
